...
The implementation builds upon the foundational code presented in [2], which employs a simple 
...
in the script Hybrid_Capacity_main.py.
...
[2] H. R. Alhennawi, M. M. El Ayadi, M. H. Ismail, and H.-A. M. Mourad,
...
def Integrand(Y, params):
    ...
    # Estimating delta[i] as in the description of eq (2) in [1]
    lower_delta = numpy.zeros(r)
    upper_delta = numpy.zeros(r)

    ''' This part was modified to make the code is capable of evaluating multiFoxH (MFHFs) when one or
    more of (a_p^(i), alpha_p^(i)) or (b_q^(i), beta_q^(i)) in Eq.8 is empty [1].'''
    i = 0
    while i < r:
        if d[i] and m[i + 1] > 0:
            dj, Dj = zip(*d[i])
        ...
        if c[i] and n[i + 1] > 0:
            ...
            Cj = numpy.array(Cj[:n[i + 1]])
            upper_delta[i] = numpy.min((1 - cj) / Cj)
        else:
            upper_delta[i] = 1
        i += 1
    mindist = numpy.linalg.norm(upper_delta - lower_delta)
    sigs = 0.5 * (upper_delta + lower_delta)

    '''This part was added to evaluate MFHFs for 'm' not equal zero in the main integral solution Eq.8 [1]'''

    if m[0] > 0:
        for j in range(m[0]):
            num = b[j][0] + numpy.sum(b[j][1:] * upper_delta)
            bnorm = numpy.linalg.norm(b[j][1:])
            newdist = numpy.abs(num) / (bnorm + numpy.finfo(float).eps)
            if newdist < mindist:
                mindist = newdist
                sigs = upper_delta - 0.5 * num * numpy.array(b[j][1:]) / (bnorm * bnorm)
    else:
        for j in range(n[0]):
            num = 1 - a[j][0] - numpy.sum(a[j][1:] * lower_delta)
            anorm = numpy.linalg.norm(a[j][1:])
            newdist = numpy.abs(num) / (anorm + numpy.finfo(float).eps)
            if newdist < mindist:
                mindist = newdist
                sigs = lower_delta + 0.5 * num * numpy.array(a[j][1:]) / (anorm * anorm)

    s += sigs

    # Computing products of Gamma factors on both numerators and denomerator
    '''This part is updated to include the products of Gamma factors in case 'm' not equal zero Eq.8 [1] '''
    sum_s = numpy.c_[numpy.ones((no_points, 1)), s]
    num = denom = 1 + 0j

    # Computing Gamma factors in eq (11) in [1]
    for j in range(n[0]):
    ...
    # Computing products of Gamma factors in eq (12) in [1]
    for i in range(r):
    ...
    # integrand over complex planes as in eq (10) in [1]
    zs = numpy.power(z, -s)
